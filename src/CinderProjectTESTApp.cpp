...
//    gl::drawSolidCircle( Vec2f( 760.0f, 560.0f ), 40.0f );
    ...
    float secX = cos( getElapsedSeconds()*5 ) * 400.0f;
    float secY = sin( getElapsedSeconds()*5 ) * 400.0f;
    glColor4f(.35f, .96f, .72f, .8f);
    gl::drawSolidCircle( Vec2f( secX, secY ) + getWindowSize() /2, 30.0f);
    
    float minX = cos( getElapsedSeconds() ) * 300.0f;
    float minY = sin( getElapsedSeconds() ) * 300.0f;
    glColor4f(.33f, .32f, .33f, .8f);
    gl::drawSolidCircle( Vec2f( minX, minY ) + getWindowSize() /2, 40.0f);
    
    float hrX = cos( getElapsedSeconds()/60 ) * 200.0f;
    float hrY = sin( getElapsedSeconds()/60 ) * 200.0f;
    glColor4f(.75, .71f, .36f, .8f);
    gl::drawSolidCircle( Vec2f( hrX, hrY ) + getWindowSize() /2, 60.0f);
    
    float dayX = cos( getElapsedSeconds()/3600 ) * 100.0f;
    float dayY = sin( getElapsedSeconds() )/3600 * 100.0f;
